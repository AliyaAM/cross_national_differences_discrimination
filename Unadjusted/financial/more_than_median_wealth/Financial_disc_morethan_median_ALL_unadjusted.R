...
library(sjPlot)
library(knitr)
Financial_disc_unadjusted_morethan_median_ALL = function(data_HRS, data_ELSA){
  
  print("Financial_disc_unadjusted_morethan_median_ALL")
   
#ELSAdiscrimination_data_wave5 = subset(data_ELSA, data_ELSA$w5wealthq ==5)
#HRS2010_discrimination_dataset = subset(data_HRS, data_HRS$Percentile_wealth_noIRA_HRS2010 == 5)
...
data_ELSA$medianWealth_ELSA = median(data_ELSA$w5wealth)
#ELSA median: 652000
#creating a new binary variable: 
data_ELSA$median_wealth_bin_ELSA = case_when(data_ELSA$w5wealth >= 652000 ~'2',
                                                                                   data_ELSA$w5wealth < 652000 ~ '1')
...
data_HRS$medianWealth_HRS = median(data_HRS$wealth_noIRA_HRS2010)
#HRS median: 787500
#creating a new binary variable: 
data_HRS$meadian_wealth_bin_HRS = case_when(data_HRS$wealth_noIRA_HRS2010 >=787500 ~ '2', 
                                                                                    data_HRS$wealth_noIRA_HRS2010 < 787500 ~ '1')

#subsetting by median wealth (less than median/more than wealth distribution's median):
ELSAdiscrimination_data_wave5 = subset(data_ELSA, data_ELSA$median_wealth_bin_ELSA == 2) 
HRS2010_discrimination_dataset = subset(data_HRS, data_HRS$meadian_wealth_bin_HRS == 2)
