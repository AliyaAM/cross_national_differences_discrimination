...
weight_obeseANDoverweight_adjusted = function(data_ELSA,
                                              data_HRS){
  


# BMI from wave 4 was used for subsetting the sample becuause there is no bmi at wave 5
ELSAdiscrimination_data_wave5 = subset(ELSAdiscrimination_data_wave5_before_subsetting, ELSAdiscrimination_data_wave5_before_subsetting$w4bmi_clean >25.0)
HRS2010_discrimination_dataset = subset(HRS2010_discrimination_dataset_before_subsetting, HRS2010_discrimination_dataset_before_subsetting$HRS2010_BMI>25.0)
...
test_weight_ANDage #sig for obese+overweight
...
test_table_weight_ANDsex # sig for obese+overweight
...
test_weight_ANDemployment #sig for obese+overweight
...
test_weight_ANDwealth_percentile #sig for obese+overweight
...
test_contengency_table_weight_ANDage_ELSA #sig for obese+overwight 
...
test_contengency_table_weight_ANDsex_ELSA #sig for obese+overwight 
...
test_contengency_table_weight_ANDemployment_ELSA #sig for obese+overweight 
...
test_contengency_table_weight_ANDwealthq_ELSA #sig for obese+overweight
...
weight_covariates_model <- glm(HRS2010_reason_discrim1_reason_weight ~continious_age + sex_1_0 + employment_allCategories +wealth_noIRA_HRS2010,  HRS2010_discrimination_dataset, family = binomial)
weight_covariates_model
summary(weight_covariates_model)


ELSA_log_reg = glm(w5weightdiscrimination2 ~ w5age + w5sex_1_0+employment + w5wealthq, data =ELSAdiscrimination_data_wave5)
...
fm1_weight <- glm(discrimination_weight ~ wealth + age + sex + employment, 
                  data = data_both_countries_weight_discrim)

fm2_weight <- glm(discrimination_weight ~  country_cat +  wealth + age + sex + employment, 
                  data = data_both_countries_weight_discrim)



cross_country_OR = exp(cbind(OR = coef(fm2_weight), confint(fm2_weight)))
...
weight_cross_national_diff = lrtest(fm1_weight, fm2_weight)

weight_chi_value_cross_national = weight_cross_national_diff$stats[1]

weight_pvalue_cross_national = weight_cross_national_diff$stats[3]
